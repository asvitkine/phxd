...
static struct SKnownBit knownbits[] = {
  //name                     offset    name                      offset
   ...
   {"send_chat",             11     }, {"initiate_private_chat", 12     },
   {"create_users",          15     }, {"delete_users",          16     },
   {"read_users",            17     }, {"modify_users",          18     },
   {"read_news",             21     }, {"post_news",             22     },
   {"disconnect_users",      23     }, {"cant_be_disconnected",  24     },
   {"get_user_info",         25     }, {"upload_anywhere",       26     },
   {"use_any_name",          27     }, {"dont_show_agreement",   28     },
   {"comment_files",         29     }, {"comment_folders",       30     },
   {"view_drop_boxes",       31     }, {"make_aliases",          32     },
   {"can_broadcast",         33     }, {"delete_articles",       34     },
   {"create_categories",     35     }, {"delete_categories",     36     },
   {"create_news_bundles",   37     }, {"delete_news_bundles",   38     },
   {"upload_folders",        39     }, {"download_folders",      40     },
   {"send_messages",         41     }, {0,0}
};
...
Options:\n\
   ...
   VALUE       1 to set the bit, 0 to unset it
Known Offset Values:\n\
...
{
   char *path;
   int bit, lowbit, highbit, ud, r, set;
   u_int8_t value, newvalue, k;
   off_t offset = 4, test;
   ...
   if (set) { /* to set the bit */
      newvalue = value |  k;
   } else { /* to unset the bit */
      newvalue = value & ~k;
   }

   /* move back to the spot where we read the byte */
   test = lseek(ud, -1, SEEK_CUR);
   if (test < 0) {
      fprintf(stderr, "lseek: %s\n", strerror(errno));
      close(ud); exit(errno); /* close file and exit */
   }

   r = write(ud, &newvalue, 1);
   if ( r != 1 ) {
