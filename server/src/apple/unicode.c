...
/* utf8to16
   ...
   function.
*/
size_t utf8to16 (const char *unicode8, u_int16_t *buf, size_t buflen)
{
#ifdef HAVE_CORESERVICES
   ...
   if (!unicode8 || !buf || !buflen) return 0;
   ...
   if (!UtoTInfo) {
      ...
      mapinfo = (UnicodeMapping *)malloc(sizeof(UnicodeMapping));
      if (!mapinfo) return -1; /* errno is set to ENOMEM */
      ...
      if (mac_errno) return -1; /* caller should consult mac_errno */
      free(mapinfo);
   }
   ...
   /* allocate a Pascal string to convert to */
   pstr = (char *)malloc(strlen(unicode8) + 1);
   if (!pstr) return -1; /* errno is set to ENOMEM */
